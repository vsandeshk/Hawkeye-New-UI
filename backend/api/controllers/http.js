...
module.exports.getData = function(req, res) {
  ...
  Promise.all([dbCon.getCategories("category_ids"), dbCon.getApplications(), dbCon.getL4(), dbCon.queryData(target_query, []), dbCon.queryData(query, [])]).then(values => {
    res_obj.categories = values[0];
    res_obj.l7 = values[1];
    res_obj.l4 = values[2];
    let target_data = values[3];
...
module.exports.searchByAnyKeyword = function(req, res) {
  ...
  let to = req.body.to;
  // query += " inner join protocol_ids on protocol_ids.protocol_id = https.protocol_l3";
  // query += " inner join protocoll4_ids on protocoll4_ids.l4_id = https.protocol_l4";
  // query += " inner join application_ids on application_ids.application_id = https.Protocol_l7";
  query+= "SELECT target_id,Source_ip,Source_port,Destination_ip,Destination_port,protocol_l3,protocol_l4,Protocol_l7,Category,Start_time,End_time,subscriber_id,operator_id,area_id FROM https ";
  query += " where ";
  let columns = ["Source_ip", "source_port", "Destination_ip", "destination_port", "Application", "url", "Category", "calling_number", "called_number", "operator_id", "area_id", "subscriber_id"];
  ...
  query += "protocol_l3 = (select protocol_id from protocol_ids where protocol_name like  \"%" + keyword + "%\" limit 1) OR "
  query += "protocol_l4 = (select l4_id from protocoll4_ids where l4_name like  \"%" + keyword + "%\" limit 1) OR ";
  query += "Protocol_l7 = (select application_id from application_ids where application_name like  \"%" + keyword + "%\" limit 1)"
  query += " AND End_time > " + from + " AND End_time < " + to + " order by https.End_time LIMIT 250 ";
